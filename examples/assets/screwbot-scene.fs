...
void main()
{
    ...
    vec3 graded = mix(vec3(luma), source.rgb, 1.10);
    ...
    float scan = 0.988 + 0.012 * sin((gl_FragCoord.y + time * 9.0) * 3.14159265);
    graded *= vec3(0.97, 1.01, 1.055) * scan;
    float emissive = max(0.0, max(graded.b, graded.g) - graded.r - 0.18);
    float instrumentPulse = 0.96 + 0.04 * sin(time * 1.7);
    graded += vec3(0.02, 0.08, 0.12) * emissive * instrumentPulse;
