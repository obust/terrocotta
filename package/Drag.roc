...
Drag := [].{
	...
	advance :
		Layout(payload), Dict(U64, List(Event.Handler(msg))), List(U64), DragState, { x : F32, y : F32, buttons : List(U8), .. } -> Try({ drag : DragState, messages : List(msg) }, Layout.LayoutError)
	...
	capture_drag :
		Layout(payload), Dict(U64, List(Event.Handler(msg))), List(U64), MouseInput -> Try({ drag : DragState, messages : List(msg) }, Layout.LayoutError)
	...
	dispatch_drag :
		Layout(payload), Dict(U64, List(Event.Handler(msg))), DragInfo, DragPhase, LayoutTypes.Pos -> Try(List(msg), Layout.LayoutError)
	...
	drag_event :
		Layout(payload), DragInfo, LayoutTypes.Pos, LayoutTypes.Pos -> Try(Event.DragEvent, Layout.LayoutError)
